...
| Assess observed product behavior | `visp review --task <id> --prepare` | Read packetPath and images; submit judgments with --session <id> --from -. |
| The slice is usable | `visp done --task <id>` | Fix concrete failures, then next for final acceptance. |
