...
ROUTES = (
    ...
    (("v1", "reservations", None), {"DELETE": "release", "GET": "get_reservation"}),
...
def make_handler(store):
    class Handler(BaseHTTPRequestHandler):
        ...
        def create_reservation(self):
            body = self.json_body()
            lines, ttl, bundle = validation.lines_request(body)
            return 201, store.reserve(lines, ttl, bundle, self.headers.get("Idempotency-Key"), body)

        def get_reservation(self, reservation_id):
            return 200, store.get_reservation(reservation_id)
